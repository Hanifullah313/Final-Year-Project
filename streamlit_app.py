...
def process_dicom_file(dicom_bytes, window_preset="Standard CXR"):
    """
    Parses raw DICOM bytes, extracts metadata, and applies Hounsfield/Window-Level normalization.
    """
    dcm = pydicom.dcmread(io.BytesIO(dicom_bytes))
    pixel_array = dcm.pixel_array.astype(np.float32)

    # Rescale Intercept and Slope to Hounsfield Units (HU)
    slope = getattr(dcm, 'RescaleSlope', 1)
    intercept = getattr(dcm, 'RescaleIntercept', 0)
    hu_image = pixel_array * slope + intercept

    # Photometric Interpretation check (Invert if MONOCHROME1)
    ...
    else:  # Standard CXR (min-max linear stretch)
...
def create_clinical_pdf(patient_data, report_text, orig_img_bytes, overlay_img_bytes, ctr_val, ctr_stat, landmarks):
    ...
    demo_table_data = [
        ...
        [
            ...
            Paragraph(f"<b>Modality:</b> DICOM PA Chest", body_text),
    ...
    img_grid = [
        [img1, img2],
        [Paragraph("Processed Digital Radiograph", caption_text), Paragraph(f"CTR Calipers & Thoracic Grounding (CTR: {ctr_val})", caption_text)]
    ]
    ...
    story.append(Paragraph(
        "<b>Notice:</b> This PACS-integrated diagnostic evaluation is generated by an Artificial Intelligence system. "
...
# --- Application Tabs ---
tab_diag, tab_history = st.tabs(["🩺 DICOM / CXR Diagnostic Analysis", "📋 Patient Records & History"])
...
window_preset = st.sidebar.selectbox(
    ...
    help="Adjusts Hounsfield Unit (HU) visualization thresholds for specific tissue density."
)
...
with tab_diag:
    ...
    st.caption("Native DICOM (.dcm) Support, Window/Level Presets, CTR Calipers & Multi-Turn AI Copilot")
    ...
    if uploaded_file is not None:
        ...
        if st.button("Run Quantitative Analysis & Report Generation", type="primary", use_container_width=True):
            with st.spinner("Executing Quantitative Inference & Calipers on GPU..."):
                try:
        ...
        if "result" in st.session_state and st.session_state["result"]:
            ...
            st.download_button(
                label="📄 Download Official DICOM-Integrated PDF Report",
                data=pdf_buffer,
            ...
            st.caption("Ask specific clinical follow-up questions about this DICOM study.")
